...
if ($_POST['Add'] == 'Add' || $_POST['Update'] == 'Update') {
      ...
      if ($err == '') {
        ...
        $bookingalerts = trim($_POST['bookingalerts']);
        if($bookingalerts == '') $bookingalerts = '0';


        if ($_POST['Update'] == 'Update') {
          $query = sprintf( 'update familymember set firstname=%s, displayname=%s, family_id=%s, email=%s, location_id=%s, workphone=%s, mobilephone=%s, skypename=%s, msnname=%s, workemail=%s, isuser=%s, admin=%s, management=%s, support=%s, bookingalerts=%s where id = %s' ,
             ...
             $famid
  	  		...
  	  		if (mhexecquery($query, $err)) {
  	  		   Header("Location: users.php?msg=" . urlencode("User Updated Successfuly") . "&sess=" . urlencode($sessionid)) ;
      		}
        }
        else {
          $query = sprintf( 'select id from familymember where email=%s' ,
  	      ...
  	      $fmid = $row['id'];
          $query = sprintf( 'insert into familymember (familymember_id, firstname, displayname, family_id, email, password, location_id, workphone, mobilephone, skypename, msnname, workemail, isuser, admin, management, support, bookingalerts) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)' ,
             $fmid,
             sqlsafe($_POST['firstname']),
             sqlsafe($displayname),
             sqlsafe($_POST['family_id']),
          ...
          if (mhexecquery($query, $err)) {
    	    	Header("Location: users.php?msg=" . urlencode("User Added Successfuly") . "&sess=" . urlencode($sessionid)) ;
          }
...
<head>
  ...
  function validateaddfamily( ) {
    ...
    if (!checkmandatoryfield(document.mainform.email, 'Email')) {
      return false;
    }
  ...
  function showaddress() {
    if ( document.all) {
      ...
      if (document.mainform.location_id.selectedIndex==1) {
        if (document.getElementById('addressdet').style.display == 'none') {
          document.getElementById('addressdet').style.display = "inline";
		  document.mainform.address1.focus();
        }
        else {
          document.getElementById('addressdet').style.display= "none";
        }
      }
      else {
        document.getElementById('addressdet').style.display= "none";
    ...
    <tr valign="top">
      <td>
        <table width="600" cellpadding="0" cellspacing="0" border="0">
          <tr valign="top">
            <td width="20">&nbsp;</td>
            <td width="562" align="left" ><h2>Add or Update User Details</h2></td>
            <td width="18">&nbsp;</td>
          </tr>
          <tr valign="top">
            <td width="20">&nbsp;</td>
...
<?}?>
                ...
                <tr valign="top">
                  ...
                  <td align="left" width="400"><? createinput_text("email", getformvalue('email', $row, !$family ), "30", "100")?>&nbsp;<span class="error">*</span></td>
                  <td align="left" width="50">&nbsp;</td>
                </tr>
                <tr valign="top">
                ...
                <tr valign="top">
                  <td align="left" width="150"><b>Receive Booking Alert Emails?:</b></td>
                  <td align="left" width="400"><? createinput_checkbox("bookingalerts", getformvalue('bookingalerts', $row, !$family ))?>&nbsp;</td>
                  <td align="left" width="50">&nbsp;</td>
                </tr>
                <?}?>
                <tr valign="top">
