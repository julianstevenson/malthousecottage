...
if ($todate == '') {
  $todate = dateadd('m', 3, strtotime($fromdate));
}
...
function reservationmonths($months) {
   ...
   switch($months) {
    ...
    case '12':
      $htmlstr = sprintf($baselink . " | " . $basenolink, "3", "3", "3", "12") ;
      break;
    default:
      $htmlstr = sprintf($basenolink . " | " . $baselink, "3", "12", "12", "12") ;
      break;
   }
...
function showlink($linkaction, $admin, $rowemail, $sessionemail, $rowid) {
  ...
  if ($rowemail == $sessionemail || $admin==1) {
    if ($linkaction=='delete') {
       $htmlstr = '<a href="javascript:cancelreservation(' . $rowid . ')" title="Cancel reservation"><img src="image/remove.gif" width="11" height="11" border="0"></a>';
    }
...
<head>
  ...
  function editreservation(rsvid) {
  	document.location.href='reservation_add.php?resid=' + rsvid + '&sess=' + escape(getsession());
  }

  function cancelreservation(rsvid) {
    if (confirm('Are you sure you would like to delete this reservation?') ) {
                ...
                <tr valign="top">
                  <td align="left">
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr valign="middle">
                        <td align="left">
							<select name="locfilter" onchange="viewbookingrequests(this.value, document.mainform.appfilter.value)">
							   <option value="0">All</option>
							   <option value="2" <?if ($locationid=='2') echo 'selected'?>>Flayosc</option>
							   <option value="1"<?if ($locationid=='1') echo 'selected'?>>Malthouse Cottage</option>
							</select>
                            &nbsp;
							<select name="appfilter" onchange="viewbookingrequests(document.mainform.locfilter.value, this.value)">
							   <option value="">All</option>
							   <option value="1" <?if ($approved=='1') echo 'selected'?>>Approved</option>
							   <option value="0"<?if ($approved=='0') echo 'selected'?>>Rejected</option>
							   <option value="2"<?if ($approved=='2') echo 'selected'?>>Pending Approval</option>
							</select>
...
<?}?>
		...
		?>
						<tr valign="middle">
						  ...
						  <td align="center" class="listitem"><a href='javascript:alert("<? echo approverejectinfo( $row['approved'], $row['displayname'], $row['approverejectdate'] );?>")' title="<? echo htmlspecialchars(approverejectinfo( $row['approved'], $row['displayname'], $row['approverejectdate'] ))?>"><? echo  getapproveimage($row['approved']);?></a>&nbsp;</td>
<?if (getsessionadmin($sessionid)==1) { ?>
                          <td align="center" class="listitem" ><? echo showlink('edit', getsessionadmin($sessionid), $row[email], getsessionemail($sessionid), $row['reservation_id'])?>&nbsp;</td>
						  <td align="center" class="listitem" ><? echo showlink('delete', getsessionadmin($sessionid), $row[email], getsessionemail($sessionid), $row['reservation_id'])?>&nbsp;</td>
<?}?>
                        </tr>
